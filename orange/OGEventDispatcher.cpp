...
#include "OGDirector.h"
 ...
 bool EventListenerKeyboard::checkAvailable()
 {
	 ...
	 {
	//	 assert(false, "Invalid EventListenerKeyboard!");
		 return false;
	 }
 ...
 {
	 if (EventListener::init(EventListener::Type::TOUCH_ONE_BY_ONE, LISTENER_ID, nullptr))
	 {
		 return true;
	 }
 ...
 //
//
// EventDispatcher::EventListenerVector::EventListenerVector() :_sceneGraphListeners(nullptr)
// {
// }
//
...
// }
//
// void EventDispatcher::EventListenerVector::push_back(EventListener * target)
// {
//	 uPEventNode element = nullptr;
//	 OG_FIND_CALLBACK(_sceneGraphListeners, target, element);
//
//	 if (element != nullptr)
//		 return;
//
//	 uPEventNode NewNode = new uEventNode;
//	 NewNode->target = target;
//
//	 DL_APPEND(_sceneGraphListeners, NewNode);
// }
//
//
//
...
//	 {
//		 OG_FREE_EVENT_CALLBACK(_sceneGraphListeners);
...
// 	//	_listenersList.push_back(listener);
// 	
// }
//
// EventListenerCustom* EventDispatcher::addCustomEventListener(const std::string &eventName, const std::function<void(EventCustom*)>& callback)
// {
//	 auto itr = _listenerMap.find(eventName);
//	 assert(itr == _listenerMap.end());
//
...
// }
//
// void EventDispatcher::EventListenerVector::removeEvent(EventListener * target)
// {
//	 uPEventNode element = nullptr;
//
//	 OG_FIND_CALLBACK(_sceneGraphListeners, target, element);
...
//		 return;
//
//	 DL_DELETE(_sceneGraphListeners, element);
//
//	 delete element;
// }
//
// void EventDispatcher::EventListenerVector::removeEvent(Node * target)
// {
//	 uPEventNode element = nullptr;
//
//	 OG_FIND_ASSOCIATED_NODE(_sceneGraphListeners, target, element);
//
//	 if (element == nullptr)
//		 return;
//
...
// }
//
// EventDispatcher::uPEventNode EventDispatcher::EventListenerVector::findNode(Node * target)
// {
//
//	 uPEventNode element = nullptr;
//
...
//
//	 if (listener == nullptr|| _listenerMap.empty())
//		 return;
//
//	 // just return if listener is in _toRemovedListeners to avoid remove listeners more than once
//
//	 auto MapItem = _listenerMap.find(listener->getListenerID());
//
//	 if (MapItem == _listenerMap.end())
//		 return;
//
//	 MapItem->second->removeEvent(listener);
//
//	 OG_SAFE_RELEASE(listener);
//
//// 	 uPEventNode listeners = MapItem->second->_sceneGraphListeners;
//// 
//// 	 uPEventNode element = nullptr;
//// 
//// 	 OG_FINDEND(listeners, listener, element);
//// 
//// 	 if (element == nullptr)
//// 		 return;
//// 
//// 	 OG_DELETE(listeners, element);
//	//delete element;
//
// }
//
...
// {
//	 if (target == nullptr || _listenerMap.empty())
//		 return;
//
//	 // just return if listener is in _toRemovedListeners to avoid remove listeners more than once
//
...
//	 {
//		 if (i.second->findNode(target) != nullptr)
//		 {
//			 i.second->removeEvent(target);
//		 }
//	 }
//
//// 
...
//// 		 return;
//// 
//// 	 MapItem->second->removeEventNode(listener);
// }
//
// /*void EventDispatcher::removeCustomEventListeners(const std::string& customEventName,
//	 std::function<void(Event *)> callbakc)
// {
//	 removeEventListenersForListenerID(customEventName, callbakc);
// }
//
...
//		 listenerItemIter = _listenerMap.erase(listenerItemIter);
//	 }
// }
//
// 
// void EventDispatcher::dispatchEventToListeners(EventListenerVector* listeners, const std::function<void(EventListener*)>& onEvent)
// {
//	 auto ret = listeners->getSceneGraphPriorityListeners();
//	 OG_TRAVEROG_EVENT(ret, onEvent);
// }
// void EventDispatcher::dispatchTouchEventToListeners(EventListenerVector* listeners, const std::function<void(EventListener*)>& onEvent)
// {
//	 auto ret = listeners->getSceneGraphPriorityListeners();
//
//	 OG_TRAVEROG_EVENT(ret, onEvent);
...
EventDispatcher::EventListenerVector::~EventListenerVector()
{
	OG_SAFE_DELETE(_sceneGraphListeners);
	OG_SAFE_DELETE(_fixedListeners);
}
...
void EventDispatcher::forceAddEventListener(EventListener* listener)
{
	...
	if (listener->getFixedPriority() == 0)
	{
		...
		assert(node != nullptr/*, "Invalid scene graph priority!"*/);
...
void EventDispatcher::addEventListenerWithSceneGraphPriority(EventListener* listener, Node* node)
{
	assert(listener && node/*, "Invalid parameters."*/);
...
void EventDispatcher::debugCheckNodeHasNoEventListenersOnDestruction(Node* node)
{
	...
	{
		assert(keyValuePair.first != node/*, "Node should have no event listeners registered for it upon destruction!"*/);

		if (keyValuePair.second)
		{
			for (EventListener * listener : *keyValuePair.second)
			{
				assert(listener->getAssociatedNode() != node/*,
					"Node should have no event listeners registered for it upon destruction!"*/);
			}
		}
	}
	...
	{
		assert(keyValuePair.first != node/*,
			"Node should have no event listeners registered for it upon destruction!"*/);
	}
...
void EventDispatcher::setPriority(EventListener* listener, int fixedPriority)
{
	...
	for (auto& iter : _listenerMap)
	{
		...
		if (fixedPriorityListeners)
		{
			...
			{
				assert(listener->getAssociatedNode() == nullptr/*, "Can't set fixed priority with scene graph based listener."*/);
...
void EventDispatcher::dispatchEventToListeners(EventListenerVector* listeners, const std::function<bool(EventListener*)>& onEvent)
{
	...
	if (fixedPriorityListeners)
	{
		assert(listeners->getGt0Index() <= static_cast<long>(fixedPriorityListeners->size())/*, "Out of range exception!"*/);
	...
	if (fixedPriorityListeners)
	{
		...
		{
			// priority > 0
			long size = fixedPriorityListeners->size();
			for (; i < size; ++i)
			{
				auto l = fixedPriorityListeners->at(i);
...
void EventDispatcher::dispatchEvent(Event* event)
{
	...
	if (event->getType() == Event::Type::KEYBOARD) {
		pfnDispatchEventToListeners = &EventDispatcher::dispatchEventToListeners;
	}
	if (event->getType() == Event::Type::MOUSE) {
		pfnDispatchEventToListeners = &EventDispatcher::dispatchTouchEventToListeners;
	}
	...
	if (oneByOneListeners)
	{
		...
		for (auto& touches : originalTouches)
		{
			...
			auto onTouchEvent = [&](EventListener* l) -> bool { // Return true to break
				...
				assert(touches->getID() == (*mutableTouchesIter)->getID()/*,
