...
{
     assert(_type != Type::VECTOR && _type != Type::MAP && _type != Type::INT_KEY_MAP/*, "Only base type (bool, string, float, double, int) could be converted"*/);
...
int Value::asInt() const
{
	...
	if (_type == Type::UNSIGNED)
	{
		assert(_field.unsignedVal < INT_MAX/*, "Can only convert values < INT_MAX"*/);
...
unsigned int Value::asUnsignedInt() const
{
	...
	if (_type == Type::INTEGER)
	{
		assert(_field.intVal >= 0/*, "Only values >= 0 can be converted to unsigned"*/);
...
float Value::asFloat() const
{
	...
	if (_type == Type::STRING)
	{
		return (float)std::atof(_field.strVal->c_str());
	}
...
{
	assert(_type == Type::INT_KEY_MAP/*, "The value type isn't Type::INT_KEY_MAP"*/);
...
void Value::clear()
{
	...
	case Type::BOOLEAN:
		...
		break;
	case Type::STRING:
		OG_SAFE_DELETE(_field.strVal);
		break;
	case Type::VECTOR:
		OG_SAFE_DELETE(_field.vectorVal);
		break;
	case Type::MAP:
		OG_SAFE_DELETE(_field.mapVal);
		break;
	case Type::INT_KEY_MAP:
		OG_SAFE_DELETE(_field.intKeyMapVal);
		break;
	default:
		break;
	}
