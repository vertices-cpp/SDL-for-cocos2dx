...
#ifndef __OGVECTOR_H__
#define __OGVECTOR_H__
...
//    }
	...
	T&  operator[](unsigned index)
	{
		/*
		return str_[index]
		当代码太多时就要用non const 版本调用const版本 */
		return const_cast<Vector<T>&>(static_cast<const Vector<T>&>(*this)[index]);
		/*
		*this对象转换成const版本:static_cast<const String&>(*this)[index]这样就可以调用const版本
		返回的是const char&,而这里返回的是char&，所以返回值还要去掉const属性const_cast<char&>(const版本调用)
		*/
	}
...
//        //CCASSERT(object != nullptr, "The object should not be nullptr");
    ...
    iterator erase(iterator position)
    {
		...
		assert(position >= _data.begin() && position < _data.end());
    ...
    iterator erase(ssize_t index)
    {
		...
		assert(!_data.empty() && index >= 0 && index < size());
