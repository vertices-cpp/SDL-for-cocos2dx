...
bool ParticleData::init(int count)
{
	...
	posx = (float*)realloc(posx,count * sizeof(float));
	posy = (float*)realloc(posy,count * sizeof(float));
	startPosX = (float*)realloc(startPosX,count * sizeof(float));
	startPosY = (float*)realloc(startPosY,count * sizeof(float));
	colorR = (float*)realloc(colorR,count * sizeof(float));
	colorG = (float*)realloc(colorG,count * sizeof(float));
	colorB = (float*)realloc(colorB,count * sizeof(float));
	colorA = (float*)realloc(colorA,count * sizeof(float));
	deltaColorR = (float*)realloc(deltaColorR,count * sizeof(float));
	deltaColorG = (float*)realloc(deltaColorG,count * sizeof(float));
	deltaColorB = (float*)realloc(deltaColorB,count * sizeof(float));
	deltaColorA = (float*)realloc(deltaColorA,count * sizeof(float));
	size = (float*)realloc(size,count * sizeof(float));
	deltaSize = (float*)realloc(deltaSize,count * sizeof(float));
	rotation = (float*)realloc(rotation,count * sizeof(float));
	deltaRotation = (float*)realloc(deltaRotation,count * sizeof(float));
	timeToLive = (float*)realloc(timeToLive,count * sizeof(float));
	atlasIndex = (unsigned int*)realloc(atlasIndex,count * sizeof(unsigned int));

	modeA.dirX = (float*)realloc(modeA.dirX,count * sizeof(float));
	modeA.dirY = (float*)realloc(modeA.dirY,count * sizeof(float));
	modeA.radialAccel = (float*)realloc(modeA.radialAccel,count * sizeof(float));
	modeA.tangentialAccel = (float*)realloc(modeA.tangentialAccel,count * sizeof(float));

	modeB.angle = (float*)realloc(modeB.angle,count * sizeof(float));
	modeB.degreesPerSecond = (float*)realloc(modeB.degreesPerSecond,count * sizeof(float));
	modeB.deltaRadius = (float*)realloc(modeB.deltaRadius,count * sizeof(float));
	modeB.radius = (float*)realloc(modeB.radius,count * sizeof(float));
...
void ParticleData::release()
{
	free(posx); posx = NULL;
	free(posy); posy = NULL;
	free(startPosX); startPosX = NULL;
	free(startPosY); startPosY = NULL;
	free(colorR); colorR = NULL;
	free(colorG); colorG = NULL;
	free(colorB); colorB = NULL;
	free(colorA); colorA = NULL;
	free(deltaColorR); deltaColorR = NULL;
	free(deltaColorG); deltaColorG = NULL;
	free(deltaColorB); deltaColorB = NULL;
	free(deltaColorA); deltaColorA = NULL;
	free(size); size = NULL;
	free(deltaSize); deltaSize = NULL;
	free(rotation); rotation = NULL;
	free(deltaRotation); deltaRotation = NULL;
	free(timeToLive); timeToLive = NULL;
	free(atlasIndex); atlasIndex = NULL;

	free(modeA.dirX); modeA.dirX = NULL;
	free(modeA.dirY); modeA.dirY = NULL;
	free(modeA.radialAccel); modeA.radialAccel = NULL;
	free(modeA.tangentialAccel); modeA.tangentialAccel = NULL;

	free(modeB.angle); modeB.angle = NULL;
	free(modeB.degreesPerSecond); modeB.degreesPerSecond = NULL;
	free(modeB.deltaRadius); modeB.deltaRadius = NULL;
	free(modeB.radius); modeB.radius = NULL;
}
...
dc[i] = (dc[i] - c[i]) / _particleData.timeToLive[i];\
    ...
    // position
	Vec2 pos = _render_position;

//     if (_positionType == PositionType::FREE)
//     {
//         pos =  Director::getInstance()->getWinSize()/2;
//     }
    ...
    if (_emitterMode == Mode::GRAVITY)
    {
        ...
        if( modeA.rotationIsDir )
        {
            for (int i = start; i < _particleCount; ++i)
            {
                ...
                _particleData.rotation[i] = -OG_RADIANS_TO_DEGREES(dir.getAngle());
            }
        }
        else
        {
...
void ParticleSystem::setTexture(std::shared_ptr<Texture2D> var)
{
    if (_texture != var)
    {
		...
		_texture = nullptr;
        _texture = var;
		_contentSize = _texture->getContentSize();
		_rect = Rect::ZERO;
		_rect.size = _contentSize;
    }
}
...
//      if(_texture)
//      {
// 		 bool premultiplied = _texture->getTexture();
//          
//          _opacityModifyRGB = false;
//          
//          if( _texture && ( _blendFunc.src == OG_BLEND_SRC && _blendFunc.dst == OG_BLEND_DST ) )
//          {
//              if( premultiplied )
//              {
...
void ParticleSystem::draw()
{
	...
	for (int i = 0; i < _particleCount; i++)
	{
		auto& p = _particleData;
		if (p.size[i] <= 0 || p.colorA[i] <= 0)
		{
			continue;
		}

		_texture->setBlendMode((SDL_BlendMode)_blendFunc);
		//SDL_SetTextureBlendMode(_texture, SDL_BLENDMODE_BLEND);
		
// 		SDL_Color c = { Uint8(p.colorR[i] * p.colorA[i] * 255), Uint8(p.colorG[i] * p.colorA[i] * 255), Uint8(p.colorB[i] * p.colorA[i] * 255),
// 			Uint8(p.colorA[i] * 255) };
// 
// 		_texture->setColor(c.r, c.g, c.b);
// 		_texture->SetTextureAlphaMod(c.a);

	//	auto scale = SDLView::getInstance()->getScale();

		Rect dest_render_position = { (p.posx[i] + p.startPosX[i] - p.size[i] / 2)/**scale.x*/ /*+ _render_position.x */,
			(p.posy[i] + p.startPosY[i] - p.size[i] / 2)/**scale.y*/ /*+ _render_position.y*/,
			(p.size[i]) /**scale.x*/, 
			(p.size[i])/**scale.y*/ };

		Color4B r = { Uint8(p.colorR[i] * p.colorA[i] * 255), Uint8(p.colorG[i] * p.colorA[i] * 255), Uint8(p.colorB[i] * p.colorA[i] * 255),
			Uint8(p.colorA[i] * 255) };
		//Rect dest_render_position = { _render_position  ,getContentSize() };
		_texture->render(_rect, dest_render_position, _rotate,
			_pointer, r, _flipX, _flipY);

// 		Texture2DManage::getInstance()->draw("__firePngData",_rect, dest_render_position, 0,
// 			_pointer, _flipX, _flipY);

	//	SDL_RenderCopyEx(SDLView::getInstance()->getRender(), _texture->getTexture(), nullptr, nullptr, p.rotation[i], nullptr, SDL_FLIP_NONE);
	}
